...
if [ -d "${NETWORK_MODELS}" ]; then
  echo "✅ Found network models at ${NETWORK_MODELS}"
  ...
  echo "⚠️  No network volume found at ${NETWORK_MODELS} (continuing with baked-in models only)"
fi

echo "────────────────────────────────────────────"
echo "🧩 Final model paths written to ${EXTRA_PATHS_FILE}:"
